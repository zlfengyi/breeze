...
int run_asr(UserData *udata, int index); //进行离线语法识别
...
int run_asr(UserData *udata, int index)
{
	...
	if (index <= 0) {
		...
		asr_audiof = buf_1;
		sprintf(asr_audiof, "wav/%d.lpcm", index);
		printf("The new lpcm file is %s", asr_audiof);
	}
	...
	if (NULL != rec_rslt) {
		char buf[200];
		sprintf(buf, "wav/%d.xml", index);
		FILE* filewrite = fopen(buf, "w");
		fprintf(filewrite, rec_rslt);
		fclose(filewrite);
	} 
	{
		char buf[200];
		sprintf(buf, "wav/%d.xml_done", index);
		FILE* filewrite = fopen(buf, "w");
...
int scan_dir(int* cur_index) {
	DIR *dir;
	struct dirent *ent;
	if ((dir = opendir ("/home/zlfengyi/breeze/Server/asr/bin/wav")) != NULL) {
	  /* print all the files and directories within directory */
	  while ((ent = readdir (dir)) != NULL) {
	  	char* name = ent->d_name;
	  	if (strlen(name) > 10) {
	  		int dot = strlen(name)-10;
	  		if (strcmp(name+dot, ".lpcm_done") != 0) continue;
	  		// printf("[zlfengyi] find a file: %s\n", name);
	  		int num = 0, i;
	  		for (i = 0; i < dot; i++) num = num*10 + name[i]-'0';
	  		if (num > *cur_index) {
	  			*cur_index = num;
	  			// printf("[zlfengyi] number is %d\n", num);
	  			return num;
	  		}
	  	} 
	  }
...
{
	// ASR initialization.
	const char *login_config    = "appid = 5636fa94"; //登录参数
	UserData asr_data; 
	int ret                     = 0 ;
	char c;
